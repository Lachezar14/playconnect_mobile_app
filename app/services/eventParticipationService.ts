...
export const fetchParticipants = async (eventId: string): Promise<Participant[]> => {
    ...
    const participantPromises = querySnapshot.docs.map(async (docSnapshot) => {
        ...
        if (!userSnapshot.empty) {
            ...
            return { id: userId, firstName: userData.firstName, lastName: userData.lastName, userRating: userData.userRating, profilePicture: userData.profilePicture } as Participant;
        }
        return null;
    });
